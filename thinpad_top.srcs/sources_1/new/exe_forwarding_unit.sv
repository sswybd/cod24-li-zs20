module exe_forwarding_unit #(
    ...
    input wire [REG_ADDR_WIDTH-1:0] mem_stage_rd_addr_i,
    output wire [1:0] forward_a_o,  // 'b00: choose #0, 'b01: #1, 'b10: #2, 'b11: choose csr
    output wire [1:0] forward_b_o   // same as `forward_a_o`
);

assign forward_a_o = (csr_wb_en_i && (mem_stage_rd_addr_i == exe_stage_operand_a_rf_addr_i) &&
                      (mem_stage_rd_addr_i != 'd0)) ? 2'b11 :
...
assign forward_b_o = (csr_wb_en_i && (mem_stage_rd_addr_i == exe_stage_operand_b_rf_addr_i) &&
                      (mem_stage_rd_addr_i != 'd0)) ? 2'b11 :
